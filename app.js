...
try {
    ...
    function renderClassifieds() {
        ...
        classifiedsList.innerHTML = filtered.map(c => `
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-md hover:shadow-lg transition-shadow" data-category="${c.category}">
                <div class="relative aspect-[4/3] cursor-pointer overflow-hidden bg-gray-100" onclick="${c.photoURL ? `document.getElementById('modal-img').src='${c.photoURL}';document.getElementById('image-modal').classList.remove('hidden');` : ''}">
                    ${c.photoURL ? `<img src="${c.photoURL}" alt="${c.title}" class="w-full h-full object-cover hover:opacity-90 transition-opacity" loading="lazy">` : '<div class="w-full h-full flex items-center justify-center text-gray-300"><svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div>'}
                    <span class="absolute top-2 right-2 text-xs px-2 py-1 rounded-full bg-blue-600 text-white font-medium shadow">${c.category}</span>
                </div>
                <div class="p-3">
                    <h4 class="font-bold text-gray-800 text-base truncate">${c.title}</h4>
                    <p class="text-green-600 font-bold text-xl mt-1">$${c.price.toLocaleString('es-AR')}</p>
                    ${c.description ? `<p class="text-gray-500 text-sm mt-2 line-clamp-2">${c.description}</p>` : ''}
                    <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                        <span class="text-xs text-gray-400">${c.userName || 'Usuario'}</span>
                        <a href="https://wa.me/${c.whatsapp}?text=${encodeURIComponent('Hola! Vi tu anuncio de "' + c.title + '" en La Bajada App')}" target="_blank" class="bg-green-500 text-white px-4 py-1.5 rounded-full text-sm font-bold hover:bg-green-600 transition-colors flex items-center gap-1.5 shadow-sm">
    ...
    function loadClassifieds() {
        ...
        onSnapshot(q, (snapshot) => {
            allClassifieds = snapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
